...
ALLOWED_PROJECTIONS = ['-TAN', '-TPV', '-TAN-SIP']
...
class WCS(object):
    ...
    def ExtractProjection(self, wcs):
        ...
        if projection not in ALLOWED_PROJECTIONS:
            ...
            err = err % (projection, ', '.join(ALLOWED_PROJECTIONS))
    ...
    def image2sph(self, xin, yin):
        ...
        w, _, _ = numpy.where(longitude < 0.0)
        if w.size > 0:
            longitude[w] += 360.0
        w, _, _ = numpy.where(longitude >= 360.0)
        if w.size > 0:
            longitude[w] -= 360.0
    ...
    def sph2image(self, longitude_in, latitude_in):
        ...
        w, _, _ = numpy.where(latitude > 0.0)
        if w.size > 0:
    ...
    def ExtractUnits(self, wcs):
        if 'cunit1' in wcs:
            ...
            if units not in ALLOWED_UNITS:
                ...
                raise ValueError(err % (units, ', '.join(ALLOWED_UNITS)))
        else:
            units = None
        return units
    ...
    def ExtractPVCoeffs(self, wcs, prefix):
        order = SCAMP_MAX_ORDER
        dim = order + 1
        matrix = numpy.zeros((dim, dim), dtype='f8')
        count = 0
        for i in range(SCAMP_MAX_NCOEFF):
            if i not in SCAMP_SKIP:
                key = prefix+'_' +str(i)
                if key in wcs:
                    indices = SCAMP_MAP[key]
...
def test_invert_2dpoly(porder, fac=5, constant=True, order_increase=0,
    ...
    if not inverse:
        ...
        w, _, _ =numpy.where((numpy.abs(x) > 5) & (numpy.abs(y) > 5))
        ...
        w, _, _ = numpy.where((numpy.abs(u) > 5) & (numpy.abs(v) > 5))
