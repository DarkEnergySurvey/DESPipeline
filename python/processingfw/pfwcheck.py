...
def check_globals(config, indent=''):
    ...
    for key in ['pipeline', 'pipeprod', 'pipever', 'project',
                'target_site', pfwdefs.SW_SITESECT,
...
def check_home_archive(config, indent=''):
    ...
    for blockname in blocklist:
        ...
        if not found_input:
            error(indent+'    ', "Could not determine %s" % (pfwdefs.USE_HOME_ARCHIVE_INPUT))
            cnts[ERRCNT_POS] += 1
        elif use_home_archive_input.lower() not in pfwdefs.VALID_HOME_ARCHIVE_INPUT:
            error(indent+'    ', "Invalid %s value" % (pfwdefs.USE_HOME_ARCHIVE_INPUT))
            cnts[ERRCNT_POS] += 1

        if not found_output:
            error(indent+'    ', "Could not determine %s" % (pfwdefs.USE_HOME_ARCHIVE_OUTPUT))
            cnts[ERRCNT_POS] += 1
        elif use_home_archive_output.lower() not in pfwdefs.VALID_HOME_ARCHIVE_OUTPUT:
            error(indent+'    ', "Invalid %s value" % (pfwdefs.USE_HOME_ARCHIVE_OUTPUT))
            cnts[ERRCNT_POS] += 1
        ...
        if ((found_input and use_home_archive_input.lower() != 'never') or
            ...
            if not found_archive:
                error(indent+'    ', "Missing %s value" % (pfwdefs.HOME_ARCHIVE))
                cnts[ERRCNT_POS] += 1
            elif pfwdefs.SW_ARCHIVESECT not in config:
                error(indent+'    ', "Needs archive section which doesn't exist")
                cnts[ERRCNT_POS] += 1
            elif home_archive not in config[pfwdefs.SW_ARCHIVESECT]:
                error(indent+'    ', "Invalid %s value" % (pfwdefs.HOME_ARCHIVE))
                cnts[ERRCNT_POS] += 1
            else:
                # check that we have all archive req values exist
                pass
...
###########################################################################
def check_filepat_valid(config, filepat, modname, objname, indent=''):
...
###########################################################################
def check_list_valid_input(modname, objname, objdict, indent=''):
...
###########################################################################
def check_exec(config, modname, dataobjs, xsectname, xsectdict, indent=''):
    ...
    try:
        cnts2 = check_exec_inputs(config, modname, dataobjs,
    ...
    try:
        cnts2 = check_exec_outputs(config, modname, dataobjs,
                                   xsectname, xsectdict, indent+'    ')
        cnts = [x + y for x, y in zip(cnts, cnts2)] # increment counts
    except:
        cnts[0] += 1
    ...
    try:
        cnts2 = check_exec_parentchild(modname, dataobjs,
                                       xsectname, xsectdict, indent+'    ')
        cnts = [x + y for x, y in zip(cnts, cnts2)] # increment counts
    except:
        cnts[0] += 1
    ...
    try:
        cnts2 = check_exec_cmd(config, modname,
                               xsectname, xsectdict, indent+'    ')
        cnts = [x + y for x, y in zip(cnts, cnts2)] # increment counts
    except:
        cnts[0] += 1
